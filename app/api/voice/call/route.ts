...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: profile } = await supabase
      .from("profiles")
      .select("preferred_voice, phone_number, timezone")
      .eq("id", user.id)
      .single()
    ...
    // Simulate call initiation (replace with real call logic)
    setTimeout(async () => {
      ...
      await supabase
        ...
        .update({
          ...
          call_transcript: `Simulated call completed successfully to ${profile.phone_number} in timezone ${profile.timezone || 'UTC'}`,
        })
        .eq("id", callLog.id)
    }, 1000)
