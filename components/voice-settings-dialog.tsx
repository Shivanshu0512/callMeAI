...
export function VoiceSettingsDialog({ children, currentVoice = "alloy", phoneNumber = "" }: VoiceSettingsDialogProps) {
  ...
  try {
      ...
      if (data.audioUrl) {
        // Always stop any currently playing audio
        // @ts-ignore
        if (window.__callMeAIAudio && typeof window.__callMeAIAudio.pause === "function") {
          // @ts-ignore
          window.__callMeAIAudio.pause()
          // @ts-ignore
          window.__callMeAIAudio.currentTime = 0
        }
        const audio = new Audio(data.audioUrl)
        // @ts-ignore
        window.__callMeAIAudio = audio
        audio.volume = 1.0
